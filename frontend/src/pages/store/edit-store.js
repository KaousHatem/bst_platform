...
const EditStore = () => {
  ...
  const [storeId, setStoreId] = useState(router.query.id && parseInt(router.query.id) || undefined)
  ...
  const handleOnSubmit = (e) => {
    ...
    setLoadingOpen(true)
    StoreProvider.updateStore(data,storeId).then(
      (response) => {
  ...
  useEffect(()=>{
    setLoadingOpen(true)
    Promise.all([
      StoreProvider.getStores(storeId),
      LocationProvider.getLocations(),
      UserProvider.getUsersShort(),
      ]).then(
      responses=>{
        setStore(responses[0].data)
                ...
                onSubmit={(event) => handleOnSubmit(event)}>
                  ...
                  <FormControl sx={{
                    ...
                    <Select
                      ...
                      label="Responsable"
                      id="demo-simple-select" 
                      defaultValue={store.store_manager}
                      // value={purchaseRequestValue}
                      // onChange={handlePurchaseRequestChange}
